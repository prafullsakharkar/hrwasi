...
/**
 * The CustomShortcuts component is responsible for rendering a list of shortcuts based on the navigation and shortcuts props.
 * It uses various MUI components to render the list items and search input.
 * The component is memoized to prevent unnecessary re-renders.
 */
function CustomShortcuts(props) {
					...
					<div className={clsx('flex flex-1', variant === 'vertical' && 'flex-col')}>
								...
								_item && (
									...
									>
										...
										>
											...
											>
												{_item.icon ? (
													<CustomSvgIcon>{_item.icon}</CustomSvgIcon>
												) : (
						...
						>
							<IconButton
								...
								size="large"
							>
								<CustomSvgIcon sx={{ color: amber[600] }}>heroicons-solid:star</CustomSvgIcon>
							</IconButton>
						</Tooltip>
					</div>
				);
		...
		>
			<MenuItem key={item.id}>
				<ListItemIcon className="min-w-40">
					{item.icon ? (
						<CustomSvgIcon>{item.icon}</CustomSvgIcon>
					) : (
...
export default memo(CustomShortcuts);
